...
@section('centro')
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h3 class="text-themecolor">Actualizar Categoría</h3>
        </div>
    ...
    <form  class="row" action="{{ url("admin/category/$category->id") }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="floating-labels m-t-40">
                         <div class="form-group {{ $errors->has('name') ? 'has-danger has-error has-feedback' : '' }} m-b-40">
                            <input type="text" class="form-control {{ $errors->has('name') ? 'form-control-danger' : '' }}" id="name" name="name" value="{{ old('name', $category->name) }}"> <br>
                         ...
                         <div class="form-group {{ $errors->has('slug') ? 'has-danger has-error has-feedback' : '' }} m-b-40">
                            ...
                            <input type="text" class="form-control {{ $errors->has('slug') ? 'form-control-danger' : '' }}" id="slug" name="slug" value="{{ old('slug',$category->slug) }}">
                            <label for="slug">Slug de la categoria (Url Amigable)</label> 
                            @if ($errors->has('slug'))
                                <span class="form-control-feedback text-danger">
                        ...
                        <button class="btn btn-success btn-rounded" type="submit">
                            <i class="fa fa-check"></i> Actualizar Categoria
                        </button>
